...
const socket = io("https://finalreadyvsr.onrender.com"); // Connect to your socket server
...
const NotesApp = ({ roomId, username, setHasNewFile }) => {
  ...
  const handleFileUpload = async (event) => {
    ...
    for (const uploadedFile of uploadedFiles) {
      ...
      try {
        const response = await axios.post("https://finalreadyvsr.onrender.com/upload-file", formData, {
  ...
  const fetchFiles = async () => {
    try {
      const response = await axios.get(`https://finalreadyvsr.onrender.com/files?roomId=${roomId}`);
  ...
  const fetchNotes = async () => {
    try {
      const response = await axios.get(`https://finalreadyvsr.onrender.com/notes?roomId=${roomId}`);
