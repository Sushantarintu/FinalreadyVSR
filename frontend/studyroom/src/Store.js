...
export const MyProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    axios.get('https://finalreadyvsr.onrender.com/getGrData')
      .then((grData) => {
  ...
  const updateData = async (newData) => {
    try {
      console.log("Previous data in context:", ldata);
      await axios.post('https://finalreadyvsr.onrender.com/curloginer', newData);
      console.log("Data sent successfully.");
  
      // Fetch the updated current user data after posting
      const response = await axios.get('https://finalreadyvsr.onrender.com/getCuruserData');
